...
class Main extends Sprite {
	...
	public function startDebugMode(e : MouseEvent) {
		...
		bitmap.scaleX = bitmap.scaleY = Math.min(maxImageHeight / bitmapData.height, maxImageWidth / bitmapData.width);
		addChild(bitmap);
		...
		var fpsCounter = UIBuilder.create(Text, {
			id : 'fpsCounter',
			left : 200,
			top  : maxImageHeight + 20});
		addChild(fpsCounter);
	...
	public function stopDebugMode(e : MouseEvent) {
		...
		UIBuilder.getAs('fpsCounter', Text).free();
	...
	public function updateLabelsAndBitmap() {
		...
		UIBuilder.getAs('counterText', Text).text = Std.string(selectedId + 1);
	}
	...
	public function tick(e : Event) {
		...
		UIBuilder.getAs('fpsCounter', Text).text = Std.string(Math.round(100 / (newTime - time)) / 100);
		time = newTime;
